...
extends Node2D
## Layered open-water movement test with terrain collision for mountain and sand artwork only.
...
func _build_lower_water_overlay() -> void:
	...
	if lower_water_texture == null:
		return
	var parallax_material: CanvasItemMaterial = _create_parallax_multiply_material()
	for tile_index: int in range(_get_horizontal_tile_count()):
		var tile_origin: Vector2 = Vector2(ENVIRONMENT_TILE_SIZE.x * float(tile_index), ENVIRONMENT_TILE_SIZE.y)
...
func _create_parallax_multiply_material() -> CanvasItemMaterial:
