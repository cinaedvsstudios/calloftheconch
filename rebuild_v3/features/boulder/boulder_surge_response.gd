...
var _surge_ram_active: bool = false
...
func _on_hylas_tail_flip_started(origin: Vector2, direction: Vector2) -> void:
	_surge_ram_active = false
	super._on_hylas_tail_flip_started(origin, direction)

func _on_hylas_surge_ram_started(_origin: Vector2, direction: Vector2, duration: float) -> void:
	if direction.length_squared() <= 0.01:
		return
	_surge_ram_active = true
...
func _apply_tail_flip_hit() -> void:
	...
	if _surge_ram_active:
		kick_speed *= surge_kick_multiplier
