...
@export_range(10.0, 150.0, 1.0) var dart_display_height: float = 58.0

@export_category("Rope Motion")
@export_range(0.0, 100.0, 1.0) var rope_wave_height: float = 22.0
@export_range(0.0, 10.0, 0.05) var rope_wave_speed: float = 2.8
...
func _update_visuals() -> void:
	var source_position: Vector2 = _dart_world_position - _direction * 20.0
	if is_instance_valid(_source):
		source_position = _source.global_position
	var local_start: Vector2 = to_local(source_position)
	var local_end: Vector2 = to_local(_dart_world_position)
	var rope_vector: Vector2 = local_end - local_start
	...
	var points: PackedVector2Array = PackedVector2Array([
		local_start,
		local_start + rope_vector * 0.33 + perpendicular * wave,
		local_start + rope_vector * 0.66 - perpendicular * wave * 0.55,
		local_end,
	])
	_rope.points = points
...
func _apply_dart_scale() -> void:
	...
	_dart_glow.scale = Vector2.ONE * scale_factor * 1.22
